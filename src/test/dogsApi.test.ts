...
describe('GET /api/dogs',()=>{
    ...
    it('Status Code 200 확인',async()=>{
        const response = await request(app).get(allTestURI);       
    ...
    it('page에 따른 데이터 확인', async()=>{
        const page1Num3Response = await request(app).get(allTestURI);
        const page1Num1Response = await request(app).get('/api/dogs?page=1&postNumInPage=1&order=latest');
        const page2Num1Response = await request(app).get('/api/dogs?page=2&postNumInPage=1&order=latest');
